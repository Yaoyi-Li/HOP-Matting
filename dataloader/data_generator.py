...
class RandomAffine(object):
    ...
    def __call__(self, sample):
        ...
        fg = cv2.warpAffine(fg, M, (cols, rows),
                            flags=maybe_random_interp(cv2.INTER_NEAREST) + cv2.WARP_INVERSE_MAP)  # , borderMode=cv2.BORDER_REFLECT)
...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        sample['alpha_shape'] = [new_h, new_w]
...
class HalfScale(object):
    def __call__(self, sample):
        h, w = sample["alpha_shape"]
        sample['image'] = cv2.resize(sample['image'], (int(w*1.5), int(h*1.5)), interpolation=cv2.INTER_NEAREST)
        sample['image'] = cv2.resize(sample['image'], (w, h), interpolation=cv2.INTER_NEAREST)
        target_h = 32 * ((h - 1) // 32 + 1)
        target_w = 32 * ((w - 1) // 32 + 1)
        pad_h = target_h - h
        pad_w = target_w - w
...
class GenTrimap(object):
    def __init__(self):
        self.erosion_kernels = [None] + [cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (size, size)) for size in range(1,50)]

    def __call__(self, sample):
        alpha = sample['alpha']

        if CONFIG.data.extreme_aug:
            fg_width = np.random.randint(0, 40)
            bg_width = np.random.randint(0, 40)
        else:
...
class DataGenerator(Dataset):
    def __init__(self, data, phase="test", val_scale="resize", test_scale="resize"):
        self.phase = phase
        ...
        self.transform = {
            'test':
                transforms.Compose(self._select_trans(test_scale))
        }[phase]
    ...
    def _select_trans(self, scale_str):
        if scale_str.lower() == "origin":
            test_trans = [ OriginScale(), ToTensor() ]
        elif scale_str.lower() == "resize":
            test_trans = [ Rescale((self.crop_size, self.crop_size)), ToTensor() ]
        elif scale_str.lower() == "crop":
            test_trans = [ RandomCrop((self.crop_size, self.crop_size)), ToTensor() ]
        elif scale_str.lower() == "interp":
            test_trans = [ HalfScale(), ToTensor() ]
        else:
            raise NotImplementedError("test_scale {} not implemented".format(scale_str))
        return test_trans
